...
func execute(delta):
	...
	var aiming_angle = Vector2.DOWN.angle() + actor.weapon_offset.global_rotation
	...
	if actor.attack_cooldown <= 0 && aim_to_player * aim_to_player < attacking_angle * attacking_angle:
