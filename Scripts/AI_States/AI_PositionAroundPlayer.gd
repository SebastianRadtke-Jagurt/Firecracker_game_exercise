...
func execute(delta):
	...
	var player_fwd = Vector2.DOWN.rotated(GameManager.player_rotation)
	var target_pos = GameManager.player_position - (player_fwd * (approached_min + approached_max)/2)
	var angle_diff = rad_to_deg(angle_difference(GameManager.player_rotation, actor.weapon_offset.global_rotation))
	...
	var angle_rotating = rad_to_deg(angle_difference(GameManager.player_rotation, actor.global_rotation))
	#Always attack if player is too close
	if dist_to_player < approached_min && actor.attack_cooldown <= 0:
		actor.set_input("attacking", true)
	#When at correct distance, attack from behind
	elif dist_to_player < approached_max:
		if angle_diff * angle_diff < angle_window * angle_window && actor.attack_cooldown <= 0:
			actor.set_input("attacking", true)
