...
func open_choice():
	...
	for i in min(player_weapons.weapons.size() - 1, 3):
		var r = randi_range(0, weapons_to_choose_idxs.size() - 1)
...
func create_weapon_choice(weapon_idx : int, weapon_texture : Texture2D):
	...
	new_weapon_choice.pressed.connect(on_choose.bind(weapon_idx), CONNECT_ONE_SHOT)
	new_weapon_choice.texture_normal = weapon_texture
	weapon_buttons_container.add_child(new_weapon_choice)
